  ...
  <div class="flex-1 p-2 sm:p-5 text-center overflow-y-auto box-border">
    ...
    <MessageBox :time-scale="timeScale" :play="play" :pause="pause" @complete="onComplete" :class="play ? 'h-full' : 'h-auto'">
        ...
        <template v-slot:keyboard>
          <div class="grid grid-cols-3 text-sm">
            <div :class="{ 'ml-1': index !== 0 }" class="text-center cursor-pointer hover:bg-gray-300 truncate p-2 rounded bg-gray-200 mt-2 box-border" v-for="(item, index) in Object.keys(merchant)">
              {{ item }}
              <KeyboardTouch v-if="index === 1" data-delay="1000" />
            </div>
          </div>
        </template>
      </MessageItem>
      ...
      <MessageItem data-remove-key="finish">
        <UserTag>{{ name.self }}</UserTag> 🟢 操作成功 <ScriptTag>build_saomaozb_20210325_133937.txt</ScriptTag>，查看狀態請執行：<br />
        <ScriptTag>/queue_1001</ScriptTag> iOS 打包狀態 <br />
      </MessageItem>
    </MessageBox>
  </div>
</template>
...
const CheckResultMessage: string = [
  ...
  "极光 Key:  <b>**</b>",
  "友盟 Key:  <b>**</b>",
  "强制邀请码:  <b>123,456,789</b>",
  "邀請碼數量:  <b>3</b>",
  "------------------------"
].join("\n")

const BuildSuccessNotify: string = [
  "🟢 打包成功 2021/03/25 14:16:53",
  "佇列: /queue_1001",
  "專案: ZB-Android-Build #248",
  "平台: Android",
  "App名称: **",
  "BundleID: **",
  "saomaozb_123_v1.19.0.apk",
  "saomaozb_456_v1.19.0.apk",
  "saomaozb_789_v1.19.0.apk"
].join("\n")
